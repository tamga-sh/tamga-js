 ...
 *
 *   ⚠️ Note also that "bytewise over UTF-8" and "code-point order" are the
 *   **same** ordering — UTF-8 is constructed so that byte comparison equals
 *   code-point comparison. There is no test to write distinguishing those two,
 *   and a port that writes one is testing nothing.
 *
 * What the sort step *can* get wrong, in any language, is two things, and both
 * are pinned by tests that were confirmed to fail when the sort is mutated:
 *
 * - **The sort key is the whole `label=value` component, not the label.** The
 *   discriminating shape is one label being a prefix of another whose next
 *   character sorts below `=` (`0x3d`): `"a-b=y"` precedes `"a=x"` because `-`
 *   is `0x2d`, while sorting by label alone puts `"a=x"` first. Exact reverses.
 * - **The sort is case-sensitive.** `"B=2"` precedes `"a=1"` (`0x42` < `0x61`);
 *   a case-folding comparator reverses that. Case folding is a defect
 *   everywhere in this algorithm, and this is its ordering half.
 *
