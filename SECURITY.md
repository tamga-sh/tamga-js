...
Three properties this surface is expected to hold:
...
- **Offline verification is tested against server-produced certificates.** `test/fixtures/machine-file-v2/` holds machine files minted by the Tamga API's own `encode_machine_file`, and `test/machine-file-server-fixtures.spec.ts` iterates their manifest. Do not add a fixture this repository generated: a builder that mirrors the verifier's own misreading of the wire format keeps CI green over a verifier that cannot open anything the server emits, which is how four defects in this path went unnoticed for two years.
- **Both offline file formats carry their expiry inside the signature, and it is enforced.** `meta.exp` is checked against the clock with a 60-second skew tolerance (`src/checkout/licenseFile.ts::verifyLicenseFileWithClaims`, `src/checkout/machineFile.ts::verifyMachineFileWithClaims`, sharing one `CLOCK_SKEW_TOLERANCE_SECONDS`), and a file whose `alg` lacks the `+v2` suffix — or whose payload carries no signed `meta` — is rejected. Under v1 the requested TTL lived only in the unsigned response envelope, so a 24-hour trial file stayed cryptographically valid forever. `alg` is not covered by the signature, so the `+v2` gate is an exact-match check that does not rely on signature validity.
