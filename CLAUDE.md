...
- **`429 TOO_MANY_REQUESTS` is live and is handled.** Credential-accepting endpoints run on a tight per-IP budget, and the calls a licensing client makes on a timer sit inside it. `src/transport.ts` parses and caps `Retry-After`, falls back to jittered exponential backoff, and retries `GET` plus five safe `POST` actions (`validate`, `validate-key`, `check-in`, `check-out`, `ping`). Creates are excluded on purpose — retrying `POST /machines` can burn a second seat. `X-RateLimit-*` response headers are still not set by any handler, so `Retry-After` is the only signal to read.
