...
- **`DEAD` is unobservable from every route this SDK calls** (M42), and would not mean "culled" even where it is visible (M5). Two layers, both of which every doc, comment and example here must respect. (1) *Unobservable:* `ping-heartbeat` writes `last_heartbeat_at = NOW()` and then computes the status from that same timestamp (`queries.rs:448-470` + `Machine::heartbeat_status_within`), so it returns `ALIVE`/`RESURRECTED` and **never** `DEAD`; `reset-heartbeat` nulls the column (`NOT_STARTED`); `POST /machines` never sets it (`NOT_STARTED`); `validate` never constructs `HEARTBEAT_DEAD`. `DEAD` only surfaces from a machine **read** (`GET /machines/{id}`, which joins the policy) — not exposed here, and adding one is M11/M36, a later turn. So a `case DEAD` branch in this repo is dead code: reframe or drop the *branch*, but never delete the `DEAD` literal from `HeartbeatStatus` or the `heartbeat_status` field — both are wire model and go live with the machine read. (2) *Not "culled":* `heartbeat_status*` never consults `require_heartbeat`, while the cull worker returns early whenever it is false (claim query requires `AND p.require_heartbeat`) and that column **defaults to `FALSE`**, so under a default policy nothing is ever culled and a machine stays `DEAD` indefinitely with its row and seat intact; a ping revives it regardless (bare `SET last_heartbeat_at = NOW()`, no resurrection check). ⚠️ **The rule to write down is the positive one:** a heartbeat scheduler must not stop, clear or short-circuit on **any** status (the stop-on-`DEAD` bug shipped in `tamga-python`), and the only terminal signal from a ping is a **`404 NOT_FOUND`**, which is where re-activation belongs. Do not justify that rule with a `DEAD`-from-a-ping scenario — it cannot happen.
